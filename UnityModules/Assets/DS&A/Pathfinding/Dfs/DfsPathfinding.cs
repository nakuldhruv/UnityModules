...
namespace Nakul.DSA
{
    public class DfsPathfinding : MonoBehaviour
    {
        ...
        private Stack<PathfindingNode>   _searchStack = new Stack<PathfindingNode>();
        ...
        private IEnumerator FindPathCoroutine()
        {
            ...
            startNode.MarkStart();
            _searchStack.Push(startNode);
            _visited.Add(startNode);

            while (_searchStack.Count > 0)
            {
                PathfindingNode node = _searchStack.Pop();
                node.MarkWalked();
